...
def shortestpath(image_matrix,V,p,q,path_type):
    ...
    #Valid_Image_Matrix contains the vertices of the nodes of th eminimal connected image graph
    #print 'Connected Undirected Graph co-ordinates=',valid_image_matrix

    #To check if the destination is in the generated valid graph
    flag_is_destination_present=0
    if q in valid_image_matrix:
        destination=q
    else:
        flag_is_destination_present=1

    if(flag_is_destination_present==1):
    ...
    path1=func(Graph1,start,end)
    Shortest_Path_Coordinates=[]
    for i in range(0,len(path1)):
        for j in range(0,len(Node_Numbers)):
            if(path1[i]==Node_Numbers[j][1]):
                Shortest_Path_Coordinates.append(Node_Numbers[j][0])

    #print Shortest_Path_Coordinates



    print 'Shortest Path Node Cordinates = ',Shortest_Path_Coordinates
    print 'Shortest Path Length=',len((path1))-1
    toc=time.time()

    print 'Code Run Time =',(toc-tic)
